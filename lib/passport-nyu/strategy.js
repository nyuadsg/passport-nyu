...
function Strategy(options, verify) {
	options = options || {};
	options.authorizationURL = options.authorizationURL || 'http://passport.sgnyuad.ompo.us/visa/oauth/authorize';
	options.tokenURL = options.tokenURL || 'http://passport.sgnyuad.ompo.us/visa/oauth/token';
...
Strategy.prototype.userProfile = function(accessToken, done) {
	this._oauth2.get('http://passport.sgnyuad.ompo.us/visa/use/info/me', accessToken, function (err, body, res) {
		...
		try {
			...
			var profile = { 'provider': 'nyu-passport' };
